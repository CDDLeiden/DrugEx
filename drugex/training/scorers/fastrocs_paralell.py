...
import sys, time, gc, threading, random
...
class FileLock:
    """A simple file-based lock for synchronizing access to cache directories."""
    ...
    def __enter__(self):
        ...
        while time.time() - start_time < self.timeout:
            try:
                ...
                self.lock_handle = os.open(self.lock_file, 
...
def _get_improved_file_hash(filepath):
    """Generate a more robust hash for a file to use as cache key."""
    try:
        stat = os.stat(filepath)
        file_size = stat.st_size
        mod_time = int(stat.st_mtime)
        
        # Include inode for additional uniqueness on Unix systems
        inode = getattr(stat, 'st_ino', 0)
        
        # Generate a hash that includes more uniqueness factors
        return f"{os.path.basename(filepath)}_{file_size}_{mod_time}_{inode}"
    except (OSError, IOError):
        return os.path.basename(filepath)

def _cleanup_old_cache_entries(max_age_days=7, min_free_space_gb=2):
    """Clean up old cache entries based on age and available disk space."""
    ...
    try:
        ...
        usage = psutil.disk_usage(os.path.dirname(_CACHE_DIR))
        free_gb = usage.free / (1024**3)
        
        # Clean up entries older than max_age_days
        cutoff_time = time.time() - (max_age_days * 86400)
        for dir_name in os.listdir(_CACHE_DIR):
            dir_path = os.path.join(_CACHE_DIR, dir_name)
            if os.path.isdir(dir_path):
                try:
                    ...
                    pass  # Skip entries with permission issues
        
        # If still need space, remove by oldest first
        if free_gb < min_free_space_gb:
            ...
            for dir_name in os.listdir(_CACHE_DIR):
                ...
                if os.path.isdir(dir_path):
                    try:
                        stat = os.stat(dir_path)
                        cache_entries.append((dir_path, stat.st_mtime))
                    except:
                        continue
            ...
            import shutil
            for dir_path, _ in cache_entries:
                try:
                    shutil.rmtree(dir_path, ignore_errors=True)
                except:
                    continue
...
def _tmpdir(prefix="fastrocs_", use_cache=False, cache_key=None):
    """Enhanced temporary directory manager with thread-safe caching."""
        ...
        try:
            with FileLock(lock_file, timeout=30):
                ...
                if not os.path.isdir(path) or not os.access(path, os.R_OK | os.W_OK):
                    path = tempfile.mkdtemp(prefix=f"{prefix}_fresh_")
                    print(f"Warning: Cache directory access issue, using fresh temp directory")
            
            # Clean up old cache entries periodically (5% chance on each access)
            if PSUTIL_AVAILABLE and random.random() < 0.05:
                _cleanup_old_cache_entries()
                
            yield path
        except Exception as e:
            print(f"Warning: Cache access error: {e}, falling back to temp directory")
            # Fall back to temp directory on any error
...
class ShapeDatabaseCache:
    ...
    def get_or_create_database(self, sq_model_path: str, use_gpu: bool) -> Tuple[oefastrocs.OEShapeDatabase, oeshape.OEShapeQuery, oefastrocs.OEShapeDatabaseOptions]:
        ...
        """
        with self.lock:
            key = f"{_get_improved_file_hash(sq_model_path)}_{use_gpu}"
...
def _process_conformer_chunk(chunk, max_iso, use_gpu, max_confs, worker_id=None):
    """
    Process a chunk of molecules for parallel conformer generation.
    
    Parameters
    ----------
    chunk : List[Tuple[str, int]]
        List of (smiles, idx) tuples to process
    max_iso : int
        Maximum number of isomers per molecule
    use_gpu : bool
        Whether to use GPU acceleration if available
    max_confs : int
        Maximum number of conformers to generate
    worker_id : int, optional
        ID of the worker process for resource management
        
    Returns
    -------
    Tuple[Dict[str, int], List[oechem.OEMol]]
        Tuple of (title_to_parent_mapping, list_of_conformers)
    """
    # Initialize worker environment if worker_id is provided
    ...
    if use_gpu:
        try:
            ...
            if not omega_gpu:
                print(f"Worker {worker_id}: GPU not ready, using CPU for conformer generation")
        except Exception as e:
            print(f"Worker {worker_id}: Error checking GPU status: {e}")
            omega_gpu = False
    
    # Create and configure Omega
    omega = oeomega.OEOmega()
    omega.SetOptions(_get_omega_options(omega_gpu, max_confs))
    omega.SetMaxConfs(max_confs)
    
    # Process molecules in this chunk
    chunk_title2parent = {}
    chunk_isomers = []
    ...
    for s, idx in chunk:
        try:
            ...
            if result:
                ...
                chunk_title2parent.update(mol_title2parent)
                chunk_isomers.extend(mol_isomers)
        except Exception as e:
            print(f"Worker {worker_id}: Error generating conformers for molecule {idx}: {e}")
            # Continue with other molecules in chunk
    
    return chunk_title2parent, chunk_isomers
...
def _prepare_molecules_for_scoring(smiles_list: List[str], idxs: List[int], 
    ...
    This implementation supports both sequential and parallel processing depending
    on the number of molecules and available system resources.
    """
    ...
    # For small batches or when using GPU, process sequentially for better stability
    if use_gpu or len(filtered_data) < 20:
        # Check if GPU is available for Omega
        omega_gpu = use_gpu
        if use_gpu:
            try:
        ...
        try:
            ...
            num_workers = max(1, min(4, _calculate_optimal_workers()))
            
            # Create chunks of molecules for parallel processing
            chunk_size = max(1, len(filtered_data) // num_workers)
            chunks = [filtered_data[i:i+chunk_size] for i in range(0, len(filtered_data), chunk_size)]
            
            print(f"Using parallel conformer generation with {num_workers} workers for {len(filtered_data)} molecules")
            
            # Use ProcessPoolExecutor with spawn context for better isolation
            ctx = mp.get_context("spawn")
            with ProcessPoolExecutor(max_workers=num_workers, mp_context=ctx) as executor:
                # Submit processing jobs for each chunk
                futures = []
                for i, chunk in enumerate(chunks):
                    future = executor.submit(
                        _process_conformer_chunk,
                        chunk=chunk,
                        max_iso=max_iso,
                        use_gpu=False,  # Disable GPU in worker processes for stability
                        max_confs=max_confs,
                        worker_id=i
                    )
                    futures.append(future)
                
                # Collect results with timeout to prevent hangs
                for future in futures:
                    try:
                        chunk_title2parent, chunk_isomers = future.result(timeout=60)
                        title2parent.update(chunk_title2parent)
                        isomers.extend(chunk_isomers)
                    except Exception as e:
                        print(f"Error in parallel conformer generation: {e}")
            ...
            omega = oeomega.OEOmega()
            omega.SetOptions(_get_omega_options(False, max_confs))  # Use CPU mode for fallback
            omega.SetMaxConfs(max_confs)
            ...
            for s, idx in filtered_data:
                try:
                    ...
                    print(f"Error in fallback processing for molecule {idx}: {e2}")
...
def _score_molecules_with_database(isomers: List[oechem.OEMol], title2parent: Dict[str, int],
    ...
    with _tmpdir(prefix="rocs_mols", use_cache=not use_gpu, 
                cache_key=_get_improved_file_hash(sq_model) if not use_gpu else None) as td:
...
def _score_batch(batch: Tuple[List[str], List[int]],
    ...
    if use_gpu:
        try:
            ...
            if not is_gpu_ready:
                print(f"Worker {worker_id}: GPU requested but FastROCS GPU is not ready, falling back to CPU")
                use_gpu = False
        except Exception as e:
            print(f"Worker {worker_id}: Error checking GPU status, falling back to CPU: {e}")
            use_gpu = False
    
    # Prepare molecules (filter, generate conformers)
    # Note: _prepare_molecules_for_scoring now has built-in parallelism for large batches
    # For small batches or when using GPU, it will still use sequential processing
    try:
        isomers, title2parent = _prepare_molecules_for_scoring(
            smiles, idxs, max_iso, max_rot, max_heavy, use_gpu, max_confs
        )
    except Exception as e:
        print(f"Worker {worker_id}: Error preparing molecules: {e}")
        return {}
...
class OpenEyeScorer(Scorer):
    ...
    def _score(self, smiles: List[str]) -> np.ndarray:
        ...
        Optimized with better resource utilization and parallel conformer generation.
        """
        if not smiles:
            return np.zeros(0)
            
        # Adjusted batch size calculation based on mode and molecule count
        if self.use_gpu:
            # GPU mode - use standard batch size
            batch_size = 30
        else:
            # CPU mode - adjust batch size based on molecule count for optimal parallelism
            if len(smiles) <= 60:
                batch_size = 15  # Smaller batches for smaller sets
            elif len(smiles) <= 200:
                batch_size = 20  # Medium batches for medium sets
            else:
                batch_size = 30  # Larger batches for large sets to benefit from parallel conformer generation
