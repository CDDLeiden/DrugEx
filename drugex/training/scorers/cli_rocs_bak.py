# This file is a backup of cli_rocs.py from DrugEx-ROCS.
# It is a previous version where the multi-molecule sdf file was crashing ROCS.
# This was fixed by splitting the multi-molecule sdf file into individual query files
# and running ROCS separately for each.
# This file is kept for reference.
# The current working version is cli_rocs.py - which has the fix and it's used.
# If you need to revert to a previous version, due the slower performance,
# you can use this file, just rename it to cli_rocs.py. 
# bug and solution is described in the file cli_rocs.py.
...
class CLIROCSScorer(Scorer):
    ...
    Attributes:
        - query_files: dict of query files for ROCS queries (.sq or molecule files).
            ...
            Supported formats: .sq (OpenEye shape query), .sdf, .mol, .pdb, etc.

            NOTE: When a multi-molecule file is provided (e.g., .sdf with multiple structures),
            ALL molecules will be used as separate queries by ROCS, and the highest score
            across all queries is returned.

        - score_type: Type of scoring to use (e.g., TanimotoCombo)
        - shape_only: If True, only shape scoring is performed
        - optimize: If True, molecular overlay optimization is performed
    ...
    def _validate_query_files(self):
        """Validate all query files exist and are readable

        NOTE: For non-.sq files, validation reads only the first molecule to check
        file validity. During ROCS execution, ALL molecules in the file will be used
        as separate queries, with the maximum score returned.
        """
        ...
        for name, list_of_qf in self.queries.items():
            ...
            for qf in list_of_qf:
                ...
                if ext == "sq":
                    ...
                    # For non-.sq files (e.g., .sdf, .mol, .pdb), validate by reading first molecule
                    # NOTE: Only the first molecule is read here for validation.
                    # ROCS CLI will use ALL molecules in the file during execution.
