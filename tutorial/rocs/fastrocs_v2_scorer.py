...
_TARGET_MEMORY_PER_WORKER = 4  # GB
...
def _get_omega_options(use_gpu: bool, max_confs: int = 10):
    """Get cached omega options for conformer generation with GPU optimization."""
    omegaOpts = oeomega.OEOmegaOptions()
    
    # Configure GPU-specific options when available
    if use_gpu:
        # GPU-optimized options based on OpenEye examples
        omegaOpts.GetTorDriveOptions().SetUseGPU(True)
        # Limit rotatable bonds for faster GPU processing
        omegaOpts.SetMaxRotors(10)
        # Faster convergence for GPU
        omegaOpts.SetRMSThreshold(0.5)
    else:
        # CPU-specific optimizations
        omegaOpts.GetTorDriveOptions().SetUseGPU(False)
        # More thorough searching for CPU mode
        omegaOpts.SetMaxRotors(8)
        omegaOpts.SetRMSThreshold(0.3)
    
    try:
        # Use fast force field for conformer generation
        omegaOpts.GetTorDriveOptions().SetForceField(oeff.OEMMFFSheffieldFFType_MMFF94s)
    except:
        pass
    
    # Other performance optimizations based on OpenEye examples
    omegaOpts.SetStrictStereo(False)  # Skip stereo checking for speed
    omegaOpts.SetFromCT(True)  # Generate from connection table
    # Limit conformers to save memory and time
    omegaOpts.SetMaxConfs(max_confs)
    
    try:
        # Enhanced energy settings for faster convergence
        omegaOpts.SetEnergyWindow(12.0)
        # Faster energy cutoffs for large molecules
        omegaOpts.SetFixDeleteH(False)
    except:
        pass
...
class ShapeDatabaseCache:
    """Class to manage persistent shape databases with efficient caching and GPU optimization."""
    
    def __init__(self):
        self.databases = {}
        self.lock = threading.RLock()
        self.gpu_databases = {}  # Store prepared GPU databases
    ...
    def get_or_create_database(self, sq_model_path: str, use_gpu: bool) -> Tuple[oefastrocs.OEShapeDatabase, oeshape.OEShapeQuery, oefastrocs.OEShapeDatabaseOptions]:
        ...
        with self.lock:
            ...
            if use_gpu:
                # GPU mode uses a single thread for best performance
                db.SetNumOpenThreads(1)
            else:
                # For CPU mode, use multiple threads
                db.SetNumOpenThreads(max(1, min(4, os.cpu_count() or 2)))
            
            # Create options with optimized mode setting
            opts = oefastrocs.OEShapeDatabaseOptions()
            if use_gpu:
                # Use GPU mode with advanced optimizations
                opts.SetFastROCSMode(oefastrocs.OEFastROCSMode_FastROCS)
                # GPU mode is already optimized for TanimotoCombo scoring
            else:
...
def _score_molecules_with_database(isomers: List[oechem.OEMol], title2parent: Dict[str, int],
    ...
    """Score molecules using a cached or newly created database - optimized for GPU performance."""
    if not isomers:
        return {}
    ...
    # GPU optimization: Direct in-memory molecule database for GPU mode
    if use_gpu:
        try:
            # The direct in-memory approach needs newer OpenEye API features
            # that might not be available. Instead, use an in-memory temporary file
            # which is still faster than disk I/O
            with tempfile.NamedTemporaryFile(suffix='.sdf') as temp_file:
                sdf_path = temp_file.name
                
                # Write molecules to in-memory SDF
                with oechem.oemolostream(sdf_path) as ofs:
                    for m in isomers:
                        # Prepare molecule for FastROCS - optimize for better scoring
                        mol_copy = oechem.OEMol(m)
                        oefastrocs.OEPrepareFastROCSMol(mol_copy) 
                        oechem.OEWriteMolecule(ofs, mol_copy)
                
                # Create molecule database from in-memory SDF
                mdb = oechem.OEMolDatabase()
                if not mdb.Open(sdf_path):
                    return {}
            
            # Create fresh database with GPU-optimized settings
            fresh_db = oefastrocs.OEShapeDatabase()
            fresh_db.SetNumOpenThreads(1)  # Single thread for GPU
            
            # Open shape database with molecule database
            if fresh_db.Open(mdb):
                # Use optimized memory management for retrieving scores
                score_iter = fresh_db.GetSortedScores(query, opts)
                result_mol = oechem.OEMol()
                
                # Process scores with optimized GPU-specific options
                for sc in score_iter:
                    mol_idx = sc.GetMolIdx() 
                    if mdb.GetMolecule(result_mol, mol_idx):
                        parent = title2parent.get(result_mol.GetTitle())
                        if parent is not None:
                            tc = sc.GetTanimotoCombo()
                            scores[parent] = max(tc, scores.get(parent, 0.0))
            
            # Clean up to help release GPU memory
            fresh_db = None
            mdb = None
            gc.collect()
            ...
            print(f"Error in GPU direct scoring: {e}, falling back to file-based method")
            # Fall back to file-based method if direct method fails
    
    # Use cache directory for database if it's CPU mode (more reusable)
    # or if GPU direct method failed
    with _tmpdir(prefix="rocs_mols", use_cache=not use_gpu, 
        ...
        with oechem.oemolostream(sdf) as ofs:
            for m in isomers:
                # Prepare for FastROCS if using GPU
                if use_gpu:
                    oefastrocs.OEPrepareFastROCSMol(m)
...
class OpenEyeScorer(Scorer):
    ...
    def __init__(self,
        ...
        # ------------------------------------------------------------------
        #   Device selection with optimized settings
        # ------------------------------------------------------------------
        ...
        if gpu_ready:
            ...
            try:
                # Ensure GPU resources are properly initialized
                if "CUDA_VISIBLE_DEVICES" in os.environ:
                    print(f"FastROCS GPU mode   : ON  (single process) - device {os.environ['CUDA_VISIBLE_DEVICES']}")
                else:
    ...
    def _score(self, smiles: List[str]) -> np.ndarray:
        ...
        # For GPU mode: use single-process scoring with optimized memory handling and batch prefetching
        if self.use_gpu:
            results = {}
            start_time = time.time()
            
            # Process batches with prefetching for better GPU utilization
            next_batch_future = None
            with ThreadPoolExecutor(max_workers=1) as prefetch_executor:
                for i, batch in enumerate(batches):
                    # Start processing next batch while current batch is being scored
                    if i < len(batches) - 1:
                        next_batch = batches[i+1]
                        # Start prefetching next batch
                        next_batch_future = prefetch_executor.submit(
                            _prepare_molecules_for_scoring,
                            next_batch[0], next_batch[1], self.max_iso, 
                    ...
                    if i % 5 == 0:
                        print(f"GPU processed {i+1}/{len(batches)} batches, " 
                              f"batch time: {batch_time:.2f}s" +
                              (f", avg time: {(time.time() - start_time) / (i+1):.2f}s per batch" if i > 0 else ""))
                    
                    # Force garbage collection to keep GPU memory available
                    if len(batch[0]) > 30:
                        gc.collect()
